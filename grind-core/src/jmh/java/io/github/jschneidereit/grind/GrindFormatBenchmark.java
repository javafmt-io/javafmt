...
/**
 * End-to-end {@link Grind#format} timing on real-shaped Java sources, parameterized
 * over both {@link PrintStrategy} implementations. {@link io.github.jschneidereit.grind.printer.PrinterFitsBenchmark}
 ...
 * </ul>
 *
 * <p>Run with {@code gradle :grind-core:jmh}.
 */
...
@Fork(1)
public class GrindFormatBenchmark {
    ...
    private GrindConfig config = GrindConfig.defaults();

    @Setup
    public void setUp() {
        source = sourceFor(corpus);
        printStrategy = strategyFor(strategy);
        config = GrindConfig.defaults();
    }

    @Benchmark
    public void formatSource(final Blackhole blackhole) {
        blackhole.consume(Grind.formatWithResult(source, config, printStrategy));
    }
